...
class BaseNormalizer:
    """
    Inherit this class and customize subclass methods to implement
    the normalization of tools.
    """
    ...
    def run(self, input_file: str):
        ...
        aro_table.set_index(
            self.preprocess_ref_genes(aro_table[self.ref_gene_col].str.lower()),
            inplace=True,
        )
    ...
    def _set_ref_gene_and_aro_cols(self):
        """
        Customize this when the reference data format is different
        from the default (e.g. for sarg orfs mode).
        """
    ...
    def get_aro_mapping_table(self):
        """
        Don't customize this unless you're using your
        own (not package built-in) reference data.
        """
        df = pd.read_csv(
            get_data_path(f"{self.tool}_{self.database}_{self.mode}_ARO_mapping.tsv"),
            sep="\t",
            index_col=0,
        )
        if self.tool != "argsoap" or self.mode != "orfs":
            df[TARGET_ARO_COL] = "ARO:" + df[TARGET_ARO_COL].astype(str).apply(
                lambda x: x.split(".")[0]
            )
        return df
...
class ARGSOAPNormalizer(BaseNormalizer):
    def __init__(self, database=None, is_hamronized=False, mode=None) -> None:
        if not database:
            ...
            warnings.warn(
                "The `database` is not supported. Will try using SARG instead."
            )
            database = "sarg"
...
class DeepARGNormalizer(BaseNormalizer):
    def __init__(self, database=None, is_hamronized=False, mode=None) -> None:
        ...
        if not database:
            ...
            warnings.warn(
                "The `database` is not supported. Will try using DeepARG instead."
            )
            database = "deeparg"
        super().__init__(database, is_hamronized, mode)
        self.tool = "deeparg"
...
class ResFinderNormalizer(BaseNormalizer):
    def __init__(self, database=None, is_hamronized=False, mode=None) -> None:
        ...
        if not database:
            ...
            warnings.warn(
                "The `database` is not supported. Will try using ResFinder instead."
            )
            database = "resfinder"
        super().__init__(database, is_hamronized, mode)
        self.tool = "resfinder"
...
class AMRFinderPlusNormalizer(BaseNormalizer):
    def __init__(self, database=None, is_hamronized=False, mode=None) -> None:
        if mode:
            warnings.warn(
                "`mode` is not relavant for AMRFinderPlus and will be ignored."
            )
            mode = "both"
        else:
            warnings.warn('`mode` is not specified. Will use default setting "both".')
        ...
        if not database:
            ...
            warnings.warn(
                "The `database` is not supported. Will try using NCBI instead."
            )
            database = "ncbi"
        super().__init__(database, is_hamronized, mode)
