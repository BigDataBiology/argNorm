...
def get_groot_argannot_db():
    ...
    for i in argannot_aro_mapping_table.index:
    ...
    preprocessed_argannot_refs = set(preprocessed_argannot_refs)
    missing_file = 'dbs/groot_missing_argannot.faa'
    with open('./dbs/argannot_groot_db.faa', 'r') as ifile, \
            open(missing_file, 'wt') as ofile:
...
def get_groot_resfinder_db():
    ...
    missing_oname = './dbs/groot_missing_resfinder.faa'
    with open('./dbs/resfinder_groot_db.fna', 'r') as ifile, \
                open(missing_oname, 'wt') as ofile:
        for record in SeqIO.parse(ifile, 'fasta'):
...
@TaskGenerator
def get_groot_missing(chunks):
    from fasta import fasta_iter

    missing_faa = 'dbs/groot_missing.faa'
    groot_missing_genes = set()
    with open(missing_faa, 'w') as ofile:
        for chunk in chunks:
            for h, seq in fasta_iter(chunk):
                groot_missing_genes.add(h)
                ofile.write(f'>{h}\n{seq}\n')
    ...
    return (ofile, groot_missing_genes)


@TaskGenerator
def combine_groot_mappings(argannot_path, resfinder_path, card_path, missing_path, groot_missing_genes):
    ...
    groot_manual_curation = pd.DataFrame(list(groot_missing_genes - set(comb_groot_mapping['Original ID'])), columns=['Original ID'])
...
def get_groot_aro_mapping():
    ...
    missing_input = get_groot_missing([argannot_input[1], resfinder_input[1] ])
    onames = combine_groot_mappings(argannot_input[0], resfinder_input[0], card_input, missing_input[0], missing_input[1])
